...
export const VerifyResetCodeBase = () => {
    ...
    const handleResendCode = async () => {
        ...
        if (!email) {
            setMessage(t('auth.emailMissing', 'Target email address missing. Please request a new code.'))
            return
        }
        ...
        try {
            ...
            if (resp && resp.status) {
                setOtpResend(resp.message || t('auth.newCodeSent', 'A new verification code has been sent to your email.'))
                setTimer(60)
            } else {
                setMessage(resp?.message || t('auth.resendFailed', 'Failed to resend verification code.'))
            }
        } catch (err: unknown) {
            ...
            setMessage(
                ...
                errorObj.message ||
                t('auth.resendFailed', 'Unable to resend verification code at this time.')
            )
        } finally {
